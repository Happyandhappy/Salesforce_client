<?php 
    ...
    if (!isset($parse_array['U']) || !isset($parse_array['P']) || !isset($parse_array['Token']) || !isset($parse_array['search_within'])){
        echo "UserName and PassWord and search_within are required!";
        exit();
    }
    ...
    try {
        ...
        $ObjType = $parse_array['search_within'];

        unset($parse_array['U']);
        unset($parse_array['P']);
        unset($parse_array['Token']);
        unset($parse_array['search_within']);

        $meta = $mySforceConnection->describeSObject($ObjType)->fields;
        ...
        if (isset($parse_array['SearchType']) && isset($parse_array['SearchValue'])){
            ...
            if (!isset($type)){
                echo "The SearchType is Unknown Field in ".$ObjType;
                exit();
            }
            ...
            if (in_array($type, $numArray))
                $query = "Select l.Id From " . $ObjType . " l where " . $parse_array['SearchType'] . "=" . $parse_array['SearchValue'] . "";
            else
                $query = "Select l.Id From " . $ObjType . " l where " . $parse_array['SearchType'] . "='" . $parse_array['SearchValue'] . "'";
        ...
        // var_dump($LeadObj);
        /* Update Lead */
        echo "*** Updating " . $ObjType . " ...\r\n";
        $updateResponse = $mySforceConnection->update(array($LeadObj), $ObjType);
        $lead_id = $updateResponse[0]->id;
        print_r($updateResponse);
        echo "<br>";

        if ($lead_id == ''){
            echo " There is error in " . $ObjType . " Pushing"; exit();
        }
